...
git clone [https://github.com/your-username/wallet-app.git](https://github.com/your-username/wallet-app.git)
cd wallet-app
...
Убедитесь, что контейнер с PostgreSQL запущен, и создайте базу для тестов:
docker exec -it wallet-app-db-1 psql -U wallet_user -d wallet_db -c "CREATE DATABASE test_wallet_db;"
...
Примечание: Таблицы в тестовой базе создаются автоматически при старте тестов через SQLAlchemy.
Что проверяют тесты:
...
Конкурентность: выполнение 20 параллельных запросов на пополнение одного кошелька без потери данных.
...
balance: В ответах возвращается строкой для предотвращения потери точности на стороне фронтенда.
